...
def estimate_expected_points_with_covariates(
    X: np.ndarray, y: np.ndarray, group_idx: int, num_groups: int
) -> float:
...
def build_forecasts(
    ...
    team_strength = (
        df_recent.groupby("team")["goals_scored"].mean().rename("team_strength")
    )
    opponent_strength = (
        df_recent.groupby("opponent_team")["goals_conceded"]
        .mean()
        .rename("opponent_weakness")
    )
    ...
    merged = merged.merge(
        opponent_strength, left_on="opponent_team", right_index=True, how="left"
    )
    merged["home"] = merged["was_home"].astype(float)

    grouped = (
        merged.groupby("element")
        .agg(
            {
                "total_points": list,
                "minutes": list,
    ...
    for _, row in tqdm(grouped.iterrows(), total=len(grouped)):
        try:
            ...
            pred = estimate_expected_points_with_covariates(
                X, y, row["group_idx"], len(group_map)
            )
...
if __name__ == "__main__":
    ...
    fixtures = pd.read_parquet(
        FIXTURES_FILE
    )  # should contain was_home and opponent_team columns
